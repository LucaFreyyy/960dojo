  ...
  useEffect(() => {
    ...
    async function fetchStreakLeaderboard() {
      const { data: streakRows, error: streakErr } = await supabase
        .from('Streak')
        .select('userId, currentStreak, longestStreak, playedToday, lastActivityDate')
        .gt('longestStreak', 0)
      ...
      const top50 = rows.map((row, index) => {
        ...
        return {
          ...
          playedToday: row.lastActivityDate === todayUtc,
        };
      });

      let nextViewerSummary = null;
      if (viewerUserId) {
        const mine = rows.find((r) => r.userId === viewerUserId);
    ...
    <>
      ...
      <main className="leaderboard-page">
        ...
        <p className="leaderboard-sub">
          {isStreakCategory
            ? 'Top 50 players by all-time daily play streak (tactics + openings, UTC days)'
        ...
        {viewerSummary && viewerSummary.mode === (isStreakCategory ? 'streak' : 'rating') ? (
          ...
          >
            ...
            {isStreakCategory && viewerSummary.kind === 'not_placed' ? (
              <p className="leaderboard-you-banner__text">
                You do not have a streak record yet. Complete a rated tactic or opening day to start your streak.
              </p>
            ) : null}
          </div>
        ) : null}
            ...
            <div className="leaderboard-state">
              {isStreakCategory ? (
                <>
                  No streak data yet.
                  <br />
                  <span className="leaderboard-state__hint">Play tactics or openings to build your daily streak.</span>
                </>
              ) : (
                <>
