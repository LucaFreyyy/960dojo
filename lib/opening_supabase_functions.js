...
export async function fetchUnfinishedOpening(userId) {
    ...
    console.log('userId in fetchUnfinishedOpening:', userId);
    ...
    console.log('fetchUnfinishedOpening result:', { data, error });
...
export async function writeBackOldOpeningAndFetchNew(userId) {
    ...
    if (updateError) {
        console.info('[writeBackOldOpeningAndFetchNew] Failed to finish old opening:', updateError);
        return;
    }
...
export async function appendEvalToDatabase(userId, evalCp, fen) {
    console.log('[appendEvalToDatabase] called with userId:', userId);
    window.WRITING_INTO_DATABASE = true;
    ...
    console.log('[appendEvalToDatabase] data:', data, 'error:', error);
