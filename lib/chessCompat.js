...
export class Chess {
  ...
  /** Standard chess draw: stalemate, dead position, or 50-move rule (100 halfmoves). */
  isDraw() {
    const p = this.pos;
    return p.isStalemate() || p.isInsufficientMaterial() || p.halfmoves >= 100;
  }
  ...
  /**
   * Apply a move in chessops UCI form (same as Lichess / engines). Handles Chess960 castling.
   */
  moveFromUci(uciStr) {
    if (!uciStr || typeof uciStr !== 'string') return null;
