...
/**
 * Apply the board move on the replayed game line, then return canonical UCI for storage.
 */
export function applyPlayMoveOrThrow(game, { from, to, promotion }) {
  const g = compatFromState(game);
  const applied = applyBoardMoveToChessGame(g, from, to, null, promotion ?? null);
  if (!applied) {
