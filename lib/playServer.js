...
async function saveGameState(redis, game) {
  await redis.set(gameStateKey(game.id), JSON.stringify(game), { EX: PLAY_GAME_TTL_SECONDS });
  if (game.status === 'active' || game.status === 'awaiting_handshake') {
    await redis.set(activeGameKey(game.whiteId), game.id, { EX: PLAY_ACTIVE_GAME_TTL_SECONDS });
    await redis.set(activeGameKey(game.blackId), game.id, { EX: PLAY_ACTIVE_GAME_TTL_SECONDS });
  }
}

async function clearActiveGame(redis, game) {
  await redis.del(activeGameKey(game.whiteId));
  await redis.del(activeGameKey(game.blackId));
}
...
async function removeUserFromQueue(redis, userId, time = null) {
  ...
  await redis.lRem(queueKey(queueTime), 0, userId);
...
async function tryMatchQueue(time) {
  ...
  try {
    while (true) {
      ...
      await redis.del(queueUserKey(first));
      await redis.del(queueUserKey(second));
...
async function getGameForUser(gameId, userId) {
  ...
  const postBoardOutcome = await resolveBoardOutcomeIfNeeded(postHandshake);
  return resolveTimeoutIfNeeded(postBoardOutcome);
}

export async function getPlayStatus(userId) {
  const redis = await getRedisClient();
  const queue = await getQueueEntryForUser(redis, userId);
  const activeGameId = await redis.get(activeGameKey(userId));
  let activeGame = null;
  if (activeGameId) {
    activeGame = await getGameForUser(activeGameId, userId);
  }
  const queuePresence = {};
  const controls = Array.isArray(PLAY_TIME_CONTROLS) ? PLAY_TIME_CONTROLS : [];
  await Promise.all(
    controls.map(async ({ time }) => {
      if (!time) return;
      const count = await redis.lLen(queueKey(time));
      queuePresence[time] = Number(count) || 0;
    })
  );
  return {
    queue,
    activeGame: activeGame ? serializeGameForClient(activeGame) : null,
    queuePresence,
  };
}
...
export async function joinPlayQueue(userId, time) {
  ...
  const activeGame = await ensureNoActiveGame(redis, userId);
  if (activeGame) {
    return { queue: null, game: activeGame, matched: true };
  }

  const currentQueueTime = await redis.get(queueUserKey(userId));
  if (currentQueueTime && currentQueueTime !== time) {
    await removeUserFromQueue(redis, userId, currentQueueTime);
  }
  if (currentQueueTime !== time) {
    await redis.rPush(queueKey(time), userId);
    await redis.expire(queueKey(time), PLAY_QUEUE_TTL_SECONDS);
    await redis.set(queueUserKey(userId), time, { EX: PLAY_QUEUE_TTL_SECONDS });
  }
...
export async function submitMove({
  ...
  const clocks = calculateDisplayClocks(game);
  const { applied, uci, compat } = applyPlayMoveOrThrow(game, { from, to, san, promotion });
...
export async function markPlayerConnected(gameId, userId) {
  ...
  const game = await getGameForUser(gameId, userId);
  if (!game) throw new Error('Game not found');
  if (game.status === 'active' || game.status === 'aborted' || game.status === 'finished' || game.status === 'draw') {
    return serializeGameForClient(game);
  }
