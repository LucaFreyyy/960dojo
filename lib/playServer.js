...
export async function joinPlayQueue(userId, time) {
  ...
  // One round trip instead of two sequential gets
  ...
  if (activeGameId) {
    const game = await getStoredGame(activeGameId);
    if (game) return { queue: null, game, matched: true };
    await redis.del(activeGameKey(userId)); // stale pointer
  }
  ...
  if (currentQueueTime !== time) {
    // Pipeline the three queue-join writes
    await redis.multi()
