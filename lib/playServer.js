...
function statusStreamCountKey(userId) {
  return `play:user:${userId}:status-stream-count`;
}

function queueLeaveDeadlineKey(userId) {
  return `play:user:${userId}:queue-leave-deadline-ms`;
}
...
function scheduleQueueLeaveResolution(userId) {
  ...
  const t = setTimeout(async () => {
    ...
    const redis = getRedisClient();
    try {
      const [countRaw, activeGameId] = await redis.mget(statusStreamCountKey(userId), activeGameKey(userId));
      const streamCount = Math.max(0, Number(countRaw) || 0);
...
export async function markStatusStreamConnected(userId) {
  ...
  const redis = getRedisClient();
  await redis.pipeline()
    .incr(statusStreamCountKey(userId))
    .expire(statusStreamCountKey(userId), 120)
    .del(queueLeaveDeadlineKey(userId))
    .exec();
}

export async function markStatusStreamDisconnected(userId) {
  const redis = getRedisClient();
  const count = await redis.decr(statusStreamCountKey(userId));
  const remaining = Math.max(0, Number(count) || 0);
  if (remaining <= 0) {
    await redis.pipeline()
      .set(statusStreamCountKey(userId), '0', { ex: 120 })
      .set(queueLeaveDeadlineKey(userId), String(Date.now() + QUEUE_DISCONNECT_GRACE_MS), {
        ex: Math.ceil((QUEUE_DISCONNECT_GRACE_MS + 30_000) / 1000),
      })
      .exec();
    scheduleQueueLeaveResolution(userId);
    return;
  }
  await redis.expire(statusStreamCountKey(userId), 120);
}
