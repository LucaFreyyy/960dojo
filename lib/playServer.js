...
const QUEUE_DISCONNECT_GRACE_MS = 8_000;

const pendingQueueLeaveTimers = new Map();
const pendingGameDisconnectTimers = new Map();
const statusStreamRefs = new Map();
...
async function createGameFromMatch(time, whiteId, blackId) {
  const redis = getRedisClient();
  const timeControl = resolvePlayTimeControl(time);
...
function clearGameDisconnectTimer(gameId) {
  const t = pendingGameDisconnectTimers.get(gameId);
  if (!t) return;
  clearTimeout(t);
  pendingGameDisconnectTimers.delete(gameId);
}

function scheduleGameDisconnectResolution(gameId) {
  clearGameDisconnectTimer(gameId);
  const t = setTimeout(async () => {
    ...
    try {
      ...
      if (!disconnectedColor || !Number.isFinite(deadlineMs) || Date.now() < deadlineMs) return;
      await abortGameDueToDisconnect(game, disconnectedColor);
    } catch (error) {
      console.error('[play/disconnect-timer]', error);
    }
  }, GAME_DISCONNECT_GRACE_MS + 250);
  if (typeof t.unref === 'function') t.unref();
  pendingGameDisconnectTimers.set(gameId, t);
}
...
export async function getPlayStatus(userId) {
  ...
  const activeGame = await (activeGameId ? getGameForUser(activeGameId, userId) : Promise.resolve(null));
...
export async function joinPlayQueue(userId, time) {
  ...
  const timeControl = resolvePlayTimeControl(time);
  if (!timeControl) throw new Error('Unsupported time control');
  const queueTime = timeControl.time;
  ...
  if (currentQueueTime && currentQueueTime !== queueTime) {
    await removeUserFromQueue(redis, userId, currentQueueTime);
  }

  if (currentQueueTime !== queueTime) {
    await redis.pipeline()
      .rpush(queueKey(queueTime), userId)
      .expire(queueKey(queueTime), PLAY_QUEUE_TTL_SECONDS)
      .set(queueUserKey(userId), queueTime, { ex: PLAY_QUEUE_TTL_SECONDS })
      .exec();
  }

  const matchedGame = await tryMatchQueue(queueTime);
  ...
  if (matchedGame) {
    ...
    await publishUserStatusEvent(redis, userId, { queue: { time: queueTime } });
  }

  return {
    queue: matchedGame ? null : { time: queueTime },
    game: matchedGame,
...
export async function markPlayerConnected(gameId, userId) {
  ...
  if (game.status === 'active') {
    const alreadyOnline =
      ...
      !game.connection?.disconnectedColor &&
      !game.connection?.deadlineAt;
    if (alreadyOnline) {
      return serializeGameForClient(game);
    }
    clearGameDisconnectTimer(gameId);
  ...
  const handshakeAlreadyReady =
    (color === 'white' ? Boolean(game.handshake?.whiteReady) : Boolean(game.handshake?.blackReady)) &&
    (color === 'white' ? Boolean(game.connection?.whiteOnline) : Boolean(game.connection?.blackOnline));
  if (handshakeAlreadyReady) {
    return serializeGameForClient(game);
  }
  await saveGameState(redis, updated);
  await publishGameEvent(redis, gameId, {
...
export async function markPlayerDisconnected(gameId, userId) {
  ...
  if (updated.status === 'active') {
    scheduleGameDisconnectResolution(gameId);
  }
  return serializeGameForClient(updated);
}

function clearQueueLeaveTimer(userId) {
  const t = pendingQueueLeaveTimers.get(userId);
  if (!t) return;
  clearTimeout(t);
  pendingQueueLeaveTimers.delete(userId);
}

function scheduleQueueLeaveResolution(userId) {
  clearQueueLeaveTimer(userId);
  const t = setTimeout(async () => {
    pendingQueueLeaveTimers.delete(userId);
    const redis = getRedisClient();
    try {
      const streamCount = Math.max(0, statusStreamRefs.get(userId) || 0);
      const activeGameId = await redis.get(activeGameKey(userId));
      if (streamCount > 0) return;
      if (activeGameId) {
        const liveGame = await getGameForUser(activeGameId, userId);
        if (liveGame && isLiveGameStatus(liveGame.status)) return;
      }
      await cancelPlayQueue(userId);
    } catch (error) {
      console.error('[play/queue-leave-timer]', error);
    }
  }, QUEUE_DISCONNECT_GRACE_MS + 250);
  if (typeof t.unref === 'function') t.unref();
  pendingQueueLeaveTimers.set(userId, t);
}

export function startPlayServerWatchdog() {
  // Intentionally a no-op: previous interval+scan watchdog created excessive Redis traffic.
  // We now resolve queue and disconnect timeouts through targeted per-user/per-game timers.
}

export async function markStatusStreamConnected(userId) {
  clearQueueLeaveTimer(userId);
  statusStreamRefs.set(userId, Math.max(0, statusStreamRefs.get(userId) || 0) + 1);
}

export async function markStatusStreamDisconnected(userId) {
  const remaining = Math.max(0, (statusStreamRefs.get(userId) || 0) - 1);
  if (remaining > 0) {
    statusStreamRefs.set(userId, remaining);
    return;
  }
  statusStreamRefs.delete(userId);
  if (remaining <= 0) {
    scheduleQueueLeaveResolution(userId);
  }
}

async function processPendingQueueLeaves(redis) {
  // Kept for compatibility; no global scan-based processing anymore.
  try {
    await redis.ping();
  } catch (error) {
    console.error('[play/processPendingQueueLeaves]', error);
  }
}
