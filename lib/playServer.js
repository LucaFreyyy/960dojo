...
export async function getPlayStatus(userId, options = {}) {
  const redis = getRedisClient();
  const includeQueuePresence = options?.includeQueuePresence !== false;
  ...
  let lengthsPromise = Promise.resolve([]);
  if (includeQueuePresence) {
    const pipeline = redis.pipeline();
    for (const { time } of controls) {
      if (time) pipeline.llen(queueKey(time));
    }
    lengthsPromise = pipeline.exec();
  }
