...
async function runPlayWatchdogTick() {
  ...
  try {
    ...
    await processPendingQueueLeaves(redis);
  } catch (error) {
...
async function processPendingQueueLeaves(redis) {
  ...
  let cursor = '0';
  do {
    const reply = await redis.scan(cursor, { match: 'play:user:*:queue-leave-deadline-ms', count: 200 });
    cursor = String(reply?.cursor || '0');
    const keys = Array.isArray(reply?.keys) ? reply.keys : [];
  ...
  await Promise.all(
    pendingKeys.map(async (key) => {
      const userId = key
        .replace(/^play:user:/, '')
        .replace(/:queue-leave-deadline-ms$/, '');
      if (!userId) return;

      const [deadlineRaw, countRaw] = await redis.mget(key, statusStreamCountKey(userId));
      ...
      if (streamCount > 0 || Date.now() < deadlineMs) return;

      const activeGameId = await redis.get(activeGameKey(userId));
      if (activeGameId) {
        const liveGame = await getGameForUser(activeGameId, userId);
        if (liveGame && isLiveGameStatus(liveGame.status)) {
          await redis.del(key);
          return;
        }
      }
