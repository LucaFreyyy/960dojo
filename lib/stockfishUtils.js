    ...
    sfJob((engine, done) => {
      function onMessage(event) {
        ...
        if (line.startsWith('bestmove')) {
          ...
          // Use chess.js to convert UCI to SAN
          const game = new Chess(fen, { chess960: true });

          const from = bestMove.slice(0, 2);  // "e2"
          const to = bestMove.slice(2, 4);    // "e4"
          const promotion = bestMove.length > 4 ? bestMove[4] : undefined;  // "q" if promotion

          const move = game.move({ from, to, promotion });

          if (!move) return resolve(null);  // Invalid move somehow

          return resolve(move.san)
        }
      }
