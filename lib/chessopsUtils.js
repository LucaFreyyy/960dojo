...
/**
 * Whether this drag is legal in the current position (catches premoves queued before the
 * position changed, e.g. into check).
 */
export function isLegalMoveFromSquares(position, fromAlg, toAlg, promotion = null) {
  if (!position) return false;
  const from = parseSquare(fromAlg);
  const to = parseSquare(toAlg);
  if (from === undefined || to === undefined) return false;
  const move = { from, to };
  const prom = normalizePromotion(promotion);
  if (prom) move.promotion = prom;
  try {
    const ctx = position.ctx();
    return position.isLegal(move, ctx);
  } catch {
    return false;
  }
}
...
function normalizePromotion(promotion) {
  ...
  const p = String(promotion).toLowerCase();
  if (p === 'q' || p === 'queen') return 'queen';
  if (p === 'r' || p === 'rook') return 'rook';
  if (p === 'b' || p === 'bishop') return 'bishop';
  if (p === 'n' || p === 'knight') return 'knight';
  return null;
}

export const makeMove = (position, from, to, promotion) => {
  ...
  if (normalizedPromotion) move.promotion = normalizedPromotion;
